...
async function executeSfCommand(command: string): Promise<{ stdout: string; stderr: string }> {
  try {
    console.error(`[${new Date().toISOString()}] Executing: ${command}`);
    const result = await execAsync(command, { 
      timeout: 300000, // 5 minute timeout
      maxBuffer: 1024 * 1024 * 10 // 10MB buffer
    });
    console.error(`[${new Date().toISOString()}] Command completed successfully`);
    return result;
  } catch (error: any) {
...
const server = new Server(
  {
    name: "salesforce-cli-mcp-docker",
    version: "1.0.0",
  },
  {
    capabilities: {
...
async function main() {
  ...
  console.error(`[${new Date().toISOString()}] Salesforce CLI MCP Server (Docker) running on stdio`);
}
